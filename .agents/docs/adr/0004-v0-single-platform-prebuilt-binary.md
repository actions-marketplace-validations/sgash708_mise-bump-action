# ADR 0004: v0はlinux向け事前ビルドバイナリのみで配布し、将来のマルチプラットフォーム移行に備えてインターフェースを固定する

> **2026-09-12追記**: GitHub-hosted linux/arm64ランナー(`ubuntu-24.04-arm`等)の利用実績が
> あるため、linux/amd64に加えてlinux/arm64向けバイナリもクロスコンパイルして同梱するように
> した(`CGO_ENABLED=0`のため追加のビルド環境は不要)。mac/windowsは引き続き対象外。
> action.yml側は`uname -m`を見て`amd64`/`arm64`のいずれかをダウンロードする。
