...
前提として、`--bump`フラグ自体も必須である。フラグ無しの`mise outdated --json`は、
ピンと同じバージョン「ファミリー」内の新版しか報告しない(例: `2.12.2`ピンには
`2.12.x`の新パッチのみが対象になり、`2.13.0`のようなマイナー更新は報告されない)ため、
実質的な更新のほとんどを見逃す。

`--bump`を付けた各エントリには`latest`(制約を無視した絶対的な最新版)と
