...
- コーディング規約(`an-internal-go-repo/.agents/docs/code-style.md`を参考に採用):
...
- ただしan-internal-go-repoのhandler/usecase/repository層別エラー変換ルール(`error-handling.md`)は、この小規模CLIには過剰なため採用しない。エラーは`fmt.Errorf`で素直にラップして上位に伝播させる

## 実データ調査で判明した前提(実装時に必ず踏まえること)

- `mise outdated --json`は**ツールキーをマップキーとするJSONオブジェクト**を返す(配列ではない)。実際に`an-internal-go-repo/mise.toml`に対して実行した結果:
  ```json
  {
...
コマンド: `cd /path/to/mise-bump-action && go mod init github.com/sgash708/mise-bump-action` を実行し、生成された`go.mod`の内容が上記と一致することを確認する(go versionは環境のものがそのまま入る。1.23系であることを確認)。
...
Run: `cd /path/to/mise-bump-action && go build ./... && golangci-lint run`
...
`internal/outdated/testdata/up_to_date.json`(an-internal-go-repoに対する実行結果そのまま。両方とも実質最新):
...
Run: `cd /path/to/mise-bump-action && go test ./internal/outdated/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/outdated/... -v`
Expected: PASS(3件とも)
...
Run: `cd /path/to/mise-bump-action && go test ./internal/misetoml/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/misetoml/... -v`
Expected: PASS(4件とも)
...
Run: `cd /path/to/mise-bump-action && go test ./internal/grouping/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/grouping/... -v`
Expected: PASS(4件とも)
...
Run: `cd /path/to/mise-bump-action && go test ./internal/prtext/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/prtext/... -v`
Expected: PASS(4件とも)
...
Run: `cd /path/to/mise-bump-action && go test ./internal/config/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/config/... -v`
Expected: PASS(3件とも)
...
Run: `cd /path/to/mise-bump-action && go run github.com/matryer/moq@v0.6.0 -out internal/runner/mocks.go ./internal/runner GitHub`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/runner/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/runner/... -v`
Expected: PASS(3件とも)
...
Run: `cd /path/to/mise-bump-action && go test ./internal/githubapi/...`
...
Run: `cd /path/to/mise-bump-action && go test ./internal/githubapi/... -v`
Expected: PASS(2件とも)
...
Run: `cd /path/to/mise-bump-action && go test ./cmd/mise-bump-action/...`
...
Run: `cd /path/to/mise-bump-action && go test ./cmd/mise-bump-action/... -v`
Expected: PASS

- [ ] **Step 5: 全パッケージのビルド・テスト・lintを通す**

Run: `cd /path/to/mise-bump-action && go build ./... && go test ./... && golangci-lint run`
Expected: すべて成功
...
Run: `cd /path/to/mise-bump-action && actionlint .github/workflows/release.yml`
